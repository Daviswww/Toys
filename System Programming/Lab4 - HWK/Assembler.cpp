...
void AsmTranslateCode(Assembler *a, AsmCode *code)
{
  ...
  switch(code->type)
  {
    ...
    case 'A':
	  ...
	  */     
	case 'Q'://2 
      if( sscanf(args, "%s %s ", p1, p2) > 0)
      {
	      sscanf(p1, "R%d", &ra);
	      sscanf(p2, "R%d", &rb);	  
	  	  sprintf(objCode, "%2x%x%x", code->opCode, ra, rb); 
	  }      
	  else
	  {
	      sscanf(args, "R%d", &ra);
	      sprintf(objCode, "%2x%x%x", code->opCode, ra, 0);
	  }    
      break; 
	case 'W'://3   
      if(sscanf(args, "%s %s %s", p1, p2, p3)> 0)
      {	
      	  /*
	      sscanf(p1, "R%d", &ra);
	      sscanf(p2, "R%d", &rb);
	      sscanf(p3, "R%d", &rc);	    	
	  	  sprintf(objCode, "%2x%x%x%x%x", code->opCode, ra, rb, rc ,0);
	  	  */
	      sscanf(p1, "R%d", &ra);
	      sscanf(p2, "R%d", &rb);
	      if(sscanf(p3, "R%d", &rc) <= 0)
	      {
	        sscanf(p3, "%d", &cx);
	        sprintf(objCode, "%2x%x%x%2x", code->opCode, ra, rb, cx);
	      }
	      else
	      {
	      	sprintf(objCode, "%2x%x%x%x%x", code->opCode, ra, rb, rc ,0);
      ...
      else
      {
	      ...
	      sprintf(objCode, "%2x%x%x%x", code->opCode, ra, rb, 0); 
	  }
      break;           
	case 'R'://4 
      if( sscanf(args, "R%d %s", &ra, p2) > 0)
	  {
	      if(strHead(p2, "["))
	      {                                                  
	        sscanf(p2, "[R%d+%s]", &rb, pt);                                       
    ...
    case 'N':
      //sprintf(objCode, "%2x%s", code->opCode, &cxCode[2]); 
      sprintf(objCode, "%2x", code->opCode); 
      break;
    default: 
      strcpy(objCode, "");
      break;
